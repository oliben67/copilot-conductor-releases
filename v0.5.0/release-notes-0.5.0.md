...
- **Self-extracting installer** — `setup-0.5.0.sh` bundles everything into a single file
...
### Installation
- `./setup-0.5.0.sh install [CONDUCTOR_HOME]` — Full installation
- `./setup-0.5.0.sh update` — Update existing installation
- `./setup-0.5.0.sh uninstall` — Clean removal (same effect as `uninstall-conductor.sh`)
- `uninstall-conductor.sh` — Standalone uninstaller (same effect as `setup-0.5.0.sh uninstall`)
...
curl -LO https://github.com/oliben67/copilot-conductor/releases/download/v0.5.0/setup-0.5.0.sh
chmod +x setup-0.5.0.sh
./setup-0.5.0.sh install
